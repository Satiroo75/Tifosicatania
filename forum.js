...
const KEY='tifosi_posts_v1';
const USER='tifosi_user_v1';
const emojis=['🔴','🔵','⚽','💙','❤️','🔥','💪','👏','🏟️','🐘','🌋','🤝','👍','👎','😂','😡'];
...
function setUser(){let nick=document.querySelector('#nick')?.value.trim()||'Utente Rossazzurro';let role=document.querySelector('#role')?.value||'utente';save(USER,{nick,role});render()}
function seed(){let posts=load(KEY,null);if(!posts){posts=[{id:crypto.randomUUID(),title:'Benvenuti sul Muro dei tifosi',body:'Questo è il primo spazio della community. Scrivi, rispondi, usa emoticon e segnala contenuti non civili. 🔴🔵',author:'SOM',role:'admin',date:now(),likes:8,reports:0,pinned:true,hidden:false,comments:[{id:crypto.randomUUID(),author:'Utente Rossazzurro',body:'Forza Catania! ⚽🔴🔵',date:now(),likes:2}]}];save(KEY,posts)}}
function addEmoji(target,e){const el=document.querySelector(target);el.value+=(el.value?' ':'')+e;el.focus()}
function publish(){let title=document.querySelector('#title').value.trim();let body=document.querySelector('#body').value.trim();if(!title||!body){alert('Inserisci titolo e messaggio.');return}let posts=load(KEY,[]);let u=user();posts.unshift({id:crypto.randomUUID(),title,body,author:u.nick,role:u.role,date:now(),likes:0,reports:0,pinned:false,hidden:false,comments:[]});save(KEY,posts);document.querySelector('#title').value='';document.querySelector('#body').value='';render()}
function like(id){let p=load(KEY,[]);let post=p.find(x=>x.id===id);post.likes++;save(KEY,p);render()}
function report(id){let p=load(KEY,[]);let post=p.find(x=>x.id===id);post.reports++;save(KEY,p);render()}
function removePost(id){if(!confirm('Eliminare questo messaggio?'))return;save(KEY,load(KEY,[]).filter(x=>x.id!==id));render()}
...
function reply(id){let txt=document.querySelector('#reply-'+id).value.trim();if(!txt)return;let p=load(KEY,[]);let post=p.find(x=>x.id===id);let u=user();post.comments.push({id:crypto.randomUUID(),author:u.nick,body:txt,date:now(),likes:0});save(KEY,p);render()}
function likeComment(pid,cid){let p=load(KEY,[]);let post=p.find(x=>x.id===pid);let c=post.comments.find(x=>x.id===cid);c.likes++;save(KEY,p);render()}
...
function render(){
  seed();
  let u=user();
  let nick=document.querySelector('#nick'); if(nick) nick.value=u.nick;
  let role=document.querySelector('#role'); if(role) role.value=u.role;
  ...
  list.innerHTML=posts.map(p=>`
    <article class="post ${p.pinned?'pinned':''} ${p.hidden?'hidden':''}">
      ...
      <div class="comments">
        ...
        <div class="row" style="justify-content:flex-start">${emojis.slice(0,10).map(e=>`<button class="btn small secondary" onclick="addEmoji('#reply-${p.id}','${e}')">${e}</button>`).join('')}<button class="btn small" onclick="reply('${p.id}')">Rispondi</button></div>
      </div>
    </article>`).join('') || '<p class="muted">Nessun messaggio trovato.</p>';
}
